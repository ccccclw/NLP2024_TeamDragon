...
def process_args(args):
    ...
    for i in range(len(task_list)):
        if task_list[i] == "cosine_similarity":
            arg_dict["cosine_similarity"] = True
        elif task_list[i] in ["surprisal","logit","output_cosine_similarity","hellinger","bhattacharyya","chisq","entropy"]:
            if not "standard_metric_list" in arg_dict:
                arg_dict["standard_metric_list"] = []
            arg_dict["standard_metric_list"].append(task_list[i])
        elif re.fullmatch(r'[lL]\d+\.*\d*', task_list[i]):
            if not "lp_norms" in arg_dict:
                arg_dict["lp_norms"] = []
            arg_dict["lp_norms"].append(task_list[i])
        elif re.fullmatch(r'[rR]enyi\d+\.*\d*', task_list[i]):
            if not "renyi" in arg_dict:
                arg_dict["renyi"] = []
            arg_dict["renyi"].append(task_list[i])
        else:
            print("Metric not supported: {}".format(task_list[i]))

    if not arg_dict["standard_metric_list"]  and not arg_dict["lp_norms"] and not arg_dict["renyi"] and not arg_dict["cosine_similarity"]:
...
def get_metric_name(metric_name):
    if metric_name=="surprisal":
        return "Surprisal"
    if metric_name=="logit":
        return "Logit"
    if metric_name=="output_cosine_similarity":
        return "OutputCosineSimilarity"
    if metric_name=="hellinger":
        return "HellingerDistance"
    if metric_name=="bhattacharyya":
        return "BhattacharyyaDistance"
    if metric_name=="chisq":
        return "ChisqDistance"
...
def create_and_run_models(arg_dict):
    ...
    slow_tokenizer_list = ["facebook/opt"]
        ...
        if arg_dict["primary_decoder"] == "masked":
            for model_name in arg_dict["model_list"]:
                ...
                try:
                    ...
                    tokenizer.add_tokens(["[!StimulusMarker!]"])
                ...
                try:
                    if revision!='[!latest!]':
                        ...
                        model = AutoModelForMaskedLM.from_pretrained(model_name,revision=str(revision), torch_dtype=config.torch_dtype)
                        model_type = "masked"
                    else:
                        config = AutoConfig.from_pretrained(model_name)
                        model = AutoModelForMaskedLM.from_pretrained(model_name, torch_dtype=config.torch_dtype)
                        model_type = "masked"
                except:
                    try:
                        if revision!='[!latest!]':
                            config = AutoConfig.from_pretrained(model_name,is_decoder=True,revision=str(revision))
                            model = AutoModelForCausalLM.from_pretrained(model_name,is_decoder=True,revision=str(revision), torch_dtype=config.torch_dtype)
                            model_type = "causal"
                        else:
                            config = AutoConfig.from_pretrained(model_name,is_decoder=True)
                            model = AutoModelForCausalLM.from_pretrained(model_name,is_decoder=True, torch_dtype=config.torch_dtype)
                            model_type = "causal"
                    except:
...
def process_stims(model,tokenizer,model_type,model_name_cleaned,arg_dict):
    for i in range(len(arg_dict["stimulus_file_list"])):
        ...
        if arg_dict["cosine_similarity"]:
            filenames["cosine_similarity"] = arg_dict["output_directory"] + "/" + stimuli_name + "." + "cosine_similarity" + "." + model_name_cleaned + "." + model_type +".output"
            metric_dict["cosine_similarity"]= []
            with open(filenames["cosine_similarity"],"w") as f:
                                f.write("FullSentence\tSentence\tTargetWords\tCosineSimilarity\tNumTokens\n")           
            with torch.no_grad():
        ...
        if arg_dict["lp_norms"]:
            for metric in arg_dict["lp_norms"] :
                filenames[metric] = arg_dict["output_directory"] + "/" + stimuli_name + "." + metric.replace(".","_") + "." + model_name_cleaned + "." + model_type +".output"
                with open(filenames[metric],"w") as f:
                    f.write("FullSentence\tSentence\tTargetWords\tL{}Norm\tNumTokens\n".format(metric[1:].replace(".","_")))
                metric_dict[metric]= []
        

        if arg_dict["renyi"]:
            for metric in arg_dict["renyi"] :
                filenames[metric] = arg_dict["output_directory"] + "/" + stimuli_name + "." + metric.replace(".","_") + "." + model_name_cleaned + "." + model_type +".output"
                with open(filenames[metric],"w") as f:
                    f.write("FullSentence\tSentence\tTargetWords\tRenyiAlpha{}\tNumTokens\n".format(metric[5:].replace(".","_")))
                metric_dict[metric]= []
        ...
        for j in range(len(stimulus_list)):
            ...
            try:
                ...
                stimulus_marker_idx = stimulus_marker_idx[0]
                ...
                if arg_dict["cosine_similarity"]:
                    embeddings_context = preceding_context
                    if arg_dict["include_following_context"]==True:
                        embeddings_context = embeddings_context + following_words                    
                    following_words
                    if embeddings_context[0]==tokenizer.bos_token_id:
                        embeddings_context = embeddings_context[1:]    
                    if embeddings_context[-1]==tokenizer.eos_token_id:
                        embeddings_context = embeddings_context[:-1]                        
                    
                    context_embeddings = model_embeddings[embeddings_context,:]
                    target_embeddings = model_embeddings[target_words,:]
                    
                    context_mean = torch.mean(context_embeddings,0)
                    target_mean =torch.mean(target_embeddings,0)
                    
                    metric_dict["cosine_similarity"].append(F.cosine_similarity(context_mean,target_mean,dim=0).item())                    
                ...
                if ("standard_metric_list" in arg_dict) or ("lp_norms" in arg_dict) or ("renyi" in arg_dict):
                    for k in range(len(target_words)):
                        ...
                        if "logit" in metric_dict:
                            logit = next_token_logits[0,current_target].item()
                            metric_dict["logit"].append(logit)

                        if arg_dict["lp_norms"]:
                            for l in range(len(arg_dict["lp_norms"])):
                                norm_name = arg_dict["lp_norms"][l]
                                norm_value = float(norm_name[1:])
                                lp_norm = F.pairwise_distance(true_dist,probability_distribution,p=norm_value).item()
                                metric_dict[norm_name].append(lp_norm)
                        if arg_dict["renyi"]:
                            for l in range(len(arg_dict["renyi"])):
                                renyi_name = arg_dict["renyi"][l]
                                renyi_alpha = float(renyi_name[5:])
                                if (not renyi_alpha==0) and (not renyi_alpha==1):
                                    renyi_prefix = torch.div(torch.tensor([[1]]).to(model.device),torch.sub(torch.tensor([[renyi_alpha]]),torch.tensor([[1]])).to(model.device))
                                    renyi_sum = torch.sum(torch.mul(torch.pow(true_dist,renyi_alpha),torch.pow(probability_distribution,torch.sub(torch.tensor([[1]]).to(model.device),renyi_alpha))))
                                    renyi = torch.mul(renyi_prefix,torch.log(renyi_sum)).item()
                                    metric_dict[renyi_name].append(renyi)
                                else:
                                    print("For Renyi divergence, alpha=0 is not implemented.\
                                         alpha=1 is equivalent to K-L divergence, \
                                            which is equivalent to surprisal in LMs. Use surprisal instead")
